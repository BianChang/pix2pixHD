import os.path
from data.base_dataset import BaseDataset, get_params, get_transform, normalize, get_transformA, get_transformB
...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.opt.no_instance:
            inst_path = self.inst_paths[index]
            inst = Image.open(inst_path)
            inst_tensor = self.transform_A(inst)

            if self.opt.load_features:
                feat_path = self.feat_paths[index]
                feat = Image.open(feat_path).convert('RGB')
                norm = normalize()
                feat_tensor = norm(self.transform_A(feat))

        input_dict = {'A': A, 'inst': inst_tensor, 'B': B,
                      'feat': feat_tensor, 'path': A_path}
